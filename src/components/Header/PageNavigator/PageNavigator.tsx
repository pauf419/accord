...
import m from "./PageNavigator.module.sass"
import { Swiper, SwiperSlide } from 'swiper/react';
import { EffectFade, Autoplay } from 'swiper/modules';
import { FC } from "react";
import { INavigatorPath } from "../../../interfaces/INavigatorPath";

interface IPageNavigatorProps {
    title: string 
    paths: INavigatorPath[]
}

const PageNavigator: FC<IPageNavigatorProps> = ({title, paths}) => {
    return (
        <div className={m.PageNavigatorWrapper}>
            <Swiper
                slidesPerView={1}
                modules={[Autoplay]}
                autoplay={{
                ...
                className="SwiperNavigator"
            >
                <SwiperSlide>
                    <div className={`${m.SwiperSlide} ${m.Slide0}`}>
                    </div>
                </SwiperSlide>
                <SwiperSlide>
                    <div className={`${m.SwiperSlide} ${m.Slide1}`}>
                        
                    </div>
                </SwiperSlide>
                <SwiperSlide>
                    <div className={`${m.SwiperSlide} ${m.Slide2}`}>
                        
                    </div>
                </SwiperSlide>
            </Swiper>
            <div className={m.NavigatorContent}>
                <div className={m.NavigatorTitle}>
                    <h1>{title}</h1>
                </div>
                <div className={m.NavigatorPath}>
                    {
                        paths.map((path:INavigatorPath, i: number) => {
                            if(i < 1) return (
                                <Link to={path.href} className={`${m.Path} ${path.active && m.Active}`}>
                                    {path.value}
                                </Link>
                            ) 
                            return (
                                <>
                                    <svg className={m.Dot} width="8px" xmlns="http://www.w3.org/2000/svg" version="1.1" id="Capa_1" viewBox="0 0 31.955 31.955">
                                        <g>
